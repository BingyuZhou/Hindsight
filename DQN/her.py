...
def her(buffer, method, n, reward_fun):
    ...
    def _her_future(buffer, k, n, reward_fun):
        ...
        future_p = 1.0 - 1.0 / (1.0 + k)

        selected_rows = random.sample(range(n), int(n * future_p))
        ...
        her_goals = buffer.buffer[selected_ind.astype(int), 2 * n + 1:3 * n +
                                  1]
        buffer.buffer[selected_ind.astype(int), n:2 * n] = her_goals
        buffer.buffer[selected_ind.astype(int), -1] = 0
    ...
    if (method == 'future'):
        return _her_future(buffer, 1, n, reward_fun)
    elif (method == 'final'):
